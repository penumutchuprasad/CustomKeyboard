...
class KeyboardLayout: NSObject, KeyboardKeyProtocol {
    ...
    func popupFrame(for key: KeyboardKey, direction: Direction) -> CGRect {
        ...
        var popupHeight = totalHeight - self.layoutConstants.popupGap - key.bounds.height
        
        for pg in model.pages {
            
            if (pg.rows.first?.contains(keyForView(key)!))! {
                popupHeight = totalHeight - self.layoutConstants.popupGap - key.bounds.height - 20
