...
let metrics: [String:Double] = ["topBanner": 30]
...
class KeyboardViewController: UIInputViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        let orientationSavvyBounds = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: self.height(forOrientation: .portrait, withTopBanner: false))
...
//                            keyView.isHidden = !needsInputModeSwitchKey
    ...
    var isContainerShowing = false {
        ...
        didSet{
            if isContainerShowing {
                self.childVCsNotif()
                self.forwardingView.isHidden = !isContainerShowing
                if let msv = self.mainStackView {
                    msv.isHidden = isContainerShowing
                }
    ...
    @objc func handleChildVCNotifs(notf: Notification) {
        ...
        if let _ = notf.object as? FinalVC {
            // Show Number Pad on View
            isNumberPadNeeded = true
            return
        }
    ...
    func showNumberPad() {
        ...
        self.forwardingView.isHidden = isNumberPadNeeded
        
        if mainStackView != nil {
            mainStackView.isHidden = !isNumberPadNeeded
        }else {
            
            mainStackView = UIStackView.init()
            ...
            self.deleteButton = KeyboardButton.init(type: .system)
            self.deleteButton.setTitle("🔙", for: .normal)
            self.deleteButton.addTarget(self, action: #selector(onBackButton_TouchUpInside), for: .touchUpInside)
            ...
            view.addConstraintsWithFormatString(formate: "H:|[v0]|", views: mainStackView)
            view.addConstraintsWithFormatString(formate: "V:[v0(250)]|", views: mainStackView)
        }
    ...
    @objc func onBackButton_TouchUpInside() {
